...
self.addEventListener('fetch', (e) => {
  ...
  if (url.pathname.endsWith('.xlsx') || url.pathname.endsWith('.docx') || url.pathname.endsWith('.pdf')) return;
  e.respondWith(
    fetch(e.request)
      .then(response => {
        if (response.ok) {
          const ctype = response.headers.get('content-type') || '';
          if (!(url.pathname.endsWith('.js') && ctype.includes('text/html'))) {
